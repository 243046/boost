...
Since the loss function $L$ can be arbitrary, algorithm presented in Algorithm~\ref{alg:gb_1} can be used in any supervised task: regression, classification or ranking. Friedman \cite{friedman_gbm} has proposed different variants of Algorithm~\ref{alg:gb_1} depending on used loss function --- he provided examples for loss functions such as Mean Squared Error or Huber loss for regression and negative binomial log-likelihood for classification. The choice of a proper loss function is critical and is strictly dependent on the type of problem which is being solved.

Additionally, a regularization hyperparameter shrinkage $0\le\nu\le 1$ in step 6 of Algorithm~\ref{alg:gb_1} has been introduced. It controls the learning rate of the algorithm and it has been shown by Friedman \cite{friedman_gbm} that its small values (such as $\nu\le 0.1$) can lead to better generalization error.

For each boosting iteration $m,\, m=1,2,\ldots,M$ the function $h(\mathbf{x}_i;\mathbf{a})$ is fit by least squares to pseudo-residuals $\tilde{y}_i$ which can be calculated by taking the negative gradient of the loss function $L$ with respect to $F$ (hence the name of the algorithm --- gradient boosting). Then, the optimal value of $\rho_m$ given in step 5 and given $h(\mathbf{x}_i;\mathbf{a})$ are used to calculate the next approximation $F_m(x)$. Without the least-squares approach, a difficult optimization problem would have to be solved instead \cite{friedman_stoch}:
\begin{equation}\label{eq:alpha_beta}
(\mathbf{a}_m, \beta_m) = \argmin_{\mathbf{a}, \beta}\sum_{i-1}^L\left(y_i,F_{m-1}(\mathbf{x}_i)+\beta  h(\mathbf{x}_i;\mathbf{a})\right)
\end{equation}

The Algorithm~\ref{alg:gb_1} can be greatly optimized when regression trees are the weak learners. The algorithm for gradient boosted decision trees have been presented in Algorithm~\ref{alg:gb_2}.
...
The algorithm for gradient boosting for binary classification with regression trees as base learners has been presented in Algorithm~\ref{alg:gb_3}. 
...
\end{algorithm}
From this moment, gradient boosting will refer to stochastic gradient boosted decision trees Algorithm presented in Algorithm~\ref{alg:gb_4}. Note that gradient boosted decision trees algorithm presented in Algorithms~\ref{alg:gb_2} and \ref{alg:gb_3} is a special case of the  stochastic variant with subsample equal to 1. 

In Friedman \cite{friedman_stoch} it has proven that introducing randomness in form of subsampling can lead to increase of performance of the gradient boosting algorithm. In addition to row-wise subsampling, feature-wise randomness can be also introduced. The original paper on stochastic gradient boosting \cite{friedman_stoch}  only introduced row subsampling, but GBM implementations often feature column subsampling. There are three hyperparameters which control feature subsampling\footnote{\url{xgboost.readthedocs.io/en/stable/parameter.html}}:
...
\end{itemize}
Column sampling in each split (\emph{colsample\_bynode}) has been originally proposed by Leo Breiman in \cite{random_forest}; it is an integral part of the Random Forest algorithm.
...
\subsection{Relevant hyperparameters available in GBM implementations}
Since each of the GBM implementations --- \gbm has a wide pool of available hyperparameters, it is reasonable to emphasize the most important ones for each implementation. A summary has been presented in Table~\ref{tab:relevant_params}.
\begin{table}[h!]
\centering
\resizebox{450pt}{!}{%
...
\end{table}
Since each implementation is different, each one of them will have a set of unique hyperparameters which are unavailable in other ones. The hyperparameters presented in Table~\ref{tab:relevant_params} as well as their specific values which will be used will be discussed in Sections~\ref{section:baseline}, \ref{section_tpe} and \ref{section_rand}.
...
Immediately, a vast improvement of gradient boosting and LightGBM can be noticed. Both of the models benefited greatly from hyperparameter tuning and right now all four models: \gbm perform very similarly. Moreover, an increase in LightGBM's performance can be observed; corresponding red boxes are much shorter --- it is possible that the addition of regularization has contributed to smaller of accuracy values. A visual judgment would suggest that among tuned models, LightGBM performs the best. XGBoost is also quite robust, but CatBoost seems to have degraded in performance --- even the original GBM proposed by Friedman in 1999 \cite{friedman_gbm} seems to uniformly outperform it. The means and standard deviations of accuracy distributions for each model and dataset have been presented in Table~\ref{tab:tpe_accuracy}. 
...
Overall, it can be concluded that hyperparameter tuning using Bayesian optimization has leveled out the performances of each of the gradient boosting implementation. However, it is not certain whether if tuned models are actually better than the baseline ones presented in Section~\ref{section:baseline}. The differences of accuracy and AUC scores between the tuned and non-tuned variants of \gbm have been presented in  Table~\ref{tab:no_tuning_tpe_accuracy_diff} and ~\ref{tab:no_tuning_tpe_AUC_diff}, respectively.
